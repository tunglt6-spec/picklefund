...
async function answer(text, userId = 'unknown') {
  ...
  if (taskType === 'picklefund_business') {
    try {
      const intent = await callLiteLLM({
        taskType,
        messages: [
          { role: 'system', content: 'Ban la Maika, tro ly dieu hanh PickleFund. Phan tich y dinh ngan gon, khong bia du lieu.' },
      ...
      return callLiteLLM({
        taskType,
        messages: [
          { role: 'system', content: 'Tra loi tieng Viet tu nhien voi persona Maika. Chi su dung du lieu PickleFund API duoc cung cap.' },
  ...
  return callLiteLLM({
    taskType,
    messages: [
      { role: 'system', content: 'Ban la Maika, tro ly dieu hanh chinh cua PickleFund. Tra loi ngan gon, ro rang, bang tieng Viet.' },
      { role: 'user', content: text },
    ],
  })
}
...
app.post('/chat', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Maika dang ban, anh thu lai sau nhe.' })
  }
})
