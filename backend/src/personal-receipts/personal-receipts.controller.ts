...
export class PersonalReceiptsController {
  ...
  async findByPeriod(@Param('fundPeriodId') fundPeriodId: string, @CurrentUser() user: any) {
    return ok(await this.service.findByPeriod(fundPeriodId, user.clubId))
  }

  @Post('generate/:fundPeriodId')
  @Roles('CLUB_ADMIN')
  async generate(@Param('fundPeriodId') fundPeriodId: string, @CurrentUser() user: any) {
    return ok(await this.service.generateForPeriod(fundPeriodId, user.clubId), 'Đã tạo phiếu chi cá nhân')
  }

  @Get('member/:memberId')
  async findByMember(@Param('memberId') memberId: string, @CurrentUser() user: any) {
