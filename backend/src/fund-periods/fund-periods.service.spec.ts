...
describe('FundPeriodsService', () => {
  ...
  describe('create', () => {
    ...
    it('should create a fund period with valid data', async () => {
      mockPrisma.fundPeriod.create.mockResolvedValue({ ...basePeriod, name: 'Q2 2026' })
    ...
    it('should throw BadRequestException when endDate <= startDate', async () => {
      const badDto = { ...validDto, endDate: '2026-03-31', startDate: '2026-06-30' }

      await expect(service.create('club-1', 'user-1', badDto)).rejects.toThrow(BadRequestException)
    })
  })
  ...
  describe('summary', () => {
    it('should compute member rows using batch groupBy (no N+1)', async () => {
      mockPrisma.fundPeriod.findFirst.mockResolvedValue(basePeriod)
      mockPrisma.fundContribution.aggregate.mockResolvedValue({ _sum: { amount: new Decimal(1000000) } })
