...
export class AuthService {
  ...
  async register(dto: {
    ...
    const clubCode = await this.prisma.club.findFirst({
      where: { code: dto.club.code },
    });
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      const club = await tx.club.create({
        data: {
          name: dto.club.name,
          code: dto.club.code,
          address: dto.club.address,
          contactPhone: dto.club.contactPhone,
          contactEmail: dto.club.contactEmail,
        },
      });
      const user = await tx.user.create({
        data: {
          username: dto.admin.username,
          email: dto.admin.email || `${dto.admin.username}@picklefund.vn`,
          passwordHash: hash,
          role: 'CLUB_ADMIN',
          clubId: club.id,
        },
  ...
  async changePassword(
    ...
    if (newPassword === '123456')
      throw new BadRequestException('Không được dùng mật khẩu mặc định 123456');
