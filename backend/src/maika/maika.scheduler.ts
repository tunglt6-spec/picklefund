...
export class MaikaScheduler {
  ...
  @Cron('0 8 * * *', { name: 'maika_daily_brief', timeZone: 'Asia/Ho_Chi_Minh' })
  async runDailyBrief() {
    ...
    for (const clubId of clubIds) {
      try {
        ...
        this.logger.error(`[Maika] Daily brief failed for ${clubId}: ${err.message}`)
      }
    }
  }

  // Weekly report: every Sunday at 9:00 AM
  @Cron('0 9 * * 0', { name: 'maika_weekly_report', timeZone: 'Asia/Ho_Chi_Minh' })
  async runWeeklyReport() {
    ...
    for (const clubId of clubIds) {
      try {
        ...
        this.logger.error(`[Maika] Weekly report failed for ${clubId}: ${err.message}`)
      }
    }
  }

  // Anomaly detection: every 6 hours
  @Cron('0 */6 * * *', { name: 'maika_anomaly_detection', timeZone: 'Asia/Ho_Chi_Minh' })
  async runAnomalyDetection() {
    ...
    for (const clubId of clubIds) {
      try {
        ...
        this.logger.error(`[Maika] Anomaly detection failed for ${clubId}: ${err.message}`)
      }
    }
  }
}
