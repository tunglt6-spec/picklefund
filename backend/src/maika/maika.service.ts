...
export class MaikaService {
  ...
  constructor(
    ...
    if (apiKey) {
      ...
      this.logger.warn(
        '[Maika] GOOGLE_API_KEY not set — AI features will use fallback logic',
      );
    }
  }
  ...
  async getClubSnapshot(clubId: string): Promise<ClubSnapshot> {
    const [club, members, contributions, expenses, sessions, activePeriod] =
      await Promise.all([
  ...
  private async askAI(prompt: string, fallback: string): Promise<string> {
    ...
    if (orKey) {
      try {
        ...
        this.logger.warn(
          `[Maika] OpenRouter error: ${err.message} — using rule-based fallback`,
        );
      }
    }
    ...
    const brief: DailyBrief = {
      ...
      debtAlert:
        snap.unpaidCount > 0
          ...
          : null,
      upcomingEvents:
        snap.currentPeriodSessions > 0
          ? `${snap.currentPeriodSessions} buổi tập trong kỳ hiện tại`
          : null,
...
Ngôn ngữ chuyên nghiệp, tiếng Việt.`;
    ...
    const report: WeeklyReport = {
      ...
      highlights: [
        ...
        snap.unpaidCount > 0
          ? `${snap.unpaidCount} người chưa đóng quỹ`
          : 'Tất cả đã đóng quỹ',
      ],
  ...
  private computeAnomaliesFromSnap(
    ...
    if (snap.activeMembers > 0 && snap.unpaidCount / snap.activeMembers > 0.5) {
      anomalies.push({
        ...
        description: `Hơn 50% thành viên chưa đóng quỹ (${snap.unpaidCount}/${snap.activeMembers})`,
        severity: 'HIGH',
      });
    }
    if (snap.commonExpense > snap.commonIncome * 1.3) {
      anomalies.push({
        type: 'expense_spike',
        description: `Chi phí vượt 130% thu nhập`,
        severity: 'MEDIUM',
      });
    }
    if (snap.activeMembers < snap.totalMembers * 0.5 && snap.totalMembers > 5) {
      anomalies.push({
        type: 'low_activity',
        description: `Chỉ ${snap.activeMembers}/${snap.totalMembers} thành viên đang hoạt động`,
        severity: 'MEDIUM',
      });
    }
    return anomalies;
  ...
  computeHealthScore(snap: ClubSnapshot): HealthScoreResult {
    ...
    const finScore =
      totalIncome > 0
        ? Math.round(
            Math.min(
              25,
              (25 * Math.max(0, snap.totalAssets)) /
                Math.max(snap.commonIncome, 1),
            ),
          )
        : 0;
    const paidRatio =
      snap.activeMembers > 0
        ? (snap.activeMembers - snap.unpaidCount) / snap.activeMembers
        : 1;
    const engScore = Math.round(25 * paidRatio);
    const actScore =
      snap.currentPeriodSessions > 0
        ? Math.min(20, snap.currentPeriodSessions * 4)
        : 0;
    const goalScore =
      snap.activeMembers >= 5 ? 15 : Math.round((15 * snap.activeMembers) / 5);
    const issueScore = Math.round(
      15 *
        (1 - Math.min(snap.unpaidCount / Math.max(snap.activeMembers, 1), 1)),
    );
    const score = finScore + engScore + actScore + goalScore + issueScore;

    const recommendations: string[] = [];
    if (snap.unpaidCount > 0)
      recommendations.push(`Nhắc ${snap.unpaidCount} thành viên đóng quỹ`);
    if (snap.totalAssets < 0)
      recommendations.push('Cân đối thu chi — quỹ đang âm');
    if (snap.currentPeriodSessions < 4)
      recommendations.push('Tăng số buổi sinh hoạt');
    if (recommendations.length === 0)
      recommendations.push('CLB đang hoạt động tốt');
    ...
    return {
      ...
      interpretation:
        score >= 75 ? 'Tốt' : score >= 50 ? 'Trung bình' : 'Cần cải thiện',
      recommendations,
    };
  }
