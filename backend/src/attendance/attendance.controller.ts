...
export class AttendanceController {
  ...
  async myAttendedSessions(@CurrentUser() user: any) {
    return ok(
      await this.service.findAttendedByMember(user.memberId, user.clubId),
    );
  }
  ...
  async delete(@Param('id') id: string, @CurrentUser() user: any) {
    return ok(
      await this.service.delete(id, user.clubId),
      'Xóa buổi chơi thành công',
    );
  }
  ...
  async updateAttendance(
    ...
    @CurrentUser() user: any,
    @Body()
    body: { attendance: { memberId: string; status: 'PRESENT' | 'ABSENT' }[] },
  ) {
    return ok(
