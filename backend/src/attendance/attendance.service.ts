  ...
  async create(
    ...
    if (!dto.fundPeriodId) {
      throw new BadRequestException(
        'Cần có kỳ quỹ đang hoạt động để tạo buổi chơi',
      );
    }
    ...
    if (!period) {
      throw new BadRequestException(
        'Kỳ quỹ không tồn tại hoặc không thuộc câu lạc bộ này',
      );
    }
  ...
  async updateAttendance(
    ...
    if (attendance.length > 0) {
      const validMembers = await this.prisma.member.findMany({
        where: {
          id: { in: attendance.map((a) => a.memberId) },
          clubId,
          isDeleted: false,
        },
        select: { id: true },
