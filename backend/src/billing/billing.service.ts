...
export class BillingService {
  ...
  async getSubscription(clubId: string): Promise<SubscriptionStatus> {
    const [tierSetting, expirySetting, memberCount] = await Promise.all([
      this.prisma.systemSetting.findUnique({ where: { key: `subscription_tier_${clubId}` } }),
      this.prisma.systemSetting.findUnique({ where: { key: `subscription_expiry_${clubId}` } }),
  ...
  async upgradePlan(clubId: string, tier: PlanTier, months: number) {
    ...
    this.logger.log(`[Billing] Club ${clubId} upgraded to ${tier} until ${expiryStr}`)
  ...
  async getAiUsage(clubId: string): Promise<{ month: string; tokens: number; estimatedCostVnd: number }[]> {
