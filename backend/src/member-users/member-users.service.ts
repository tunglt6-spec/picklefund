...
export class MemberUsersService {
  ...
  async findAll(clubId: string) {
    const users = await this.prisma.user.findMany({
      ...
      select: {
        ...
        mustChangePassword: true, lastLoginAt: true, notificationEnabled: true,
        createdAt: true, member: { select: { id: true, fullName: true, phone: true, email: true } },
      },
  ...
  async create(clubId: string, adminUserId: string, dto: {
    memberId: string
    username?: string
    email?: string
    ...
    if (member.userId) {
      const existing = await this.prisma.user.findUnique({ where: { id: member.userId } })
    ...
    const user = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.member.update({ where: { id: member.id }, data: { userId: u.id } })
      await tx.auditLog.create({
        data: { clubId, userId: adminUserId, action: 'CREATE', resource: 'member_user', resourceId: u.id, detail: `Tạo tài khoản ${username} cho thành viên ${member.fullName}` },
      })
      return u
    })
  ...
  async bulkCreate(clubId: string, adminUserId: string, dto: {
    memberIds: string[]
    mustChangePassword?: boolean
    notificationEnabled?: boolean
  }) {
    const results: { memberId: string; memberName: string; username: string; success: boolean; error?: string }[] = []
  ...
  async resetPassword(accountId: string, adminUserId: string, clubId: string) {
    ...
    await this.prisma.$transaction([
      ...
      this.prisma.auditLog.create({
        data: { clubId, userId: adminUserId, action: 'RESET_PASSWORD', resource: 'member_user', resourceId: accountId, detail: `Reset mật khẩu tài khoản ${user.username}` },
      }),
    ])
    return { message: 'Đã reset mật khẩu về 123456' }
  }

  async updateStatus(accountId: string, isActive: boolean, adminUserId: string, clubId: string) {
    const user = await this.prisma.user.findFirst({
      where: { id: accountId, clubId, role: 'CLUB_MEMBER' },
    })
    if (!user) throw new NotFoundException('Tài khoản không tồn tại')
    ...
    await this.prisma.$transaction([
      ...
      this.prisma.auditLog.create({
        data: {
          clubId, userId: adminUserId,
          action: isActive ? 'UNLOCK_ACCOUNT' : 'LOCK_ACCOUNT',
          resource: 'member_user', resourceId: accountId,
