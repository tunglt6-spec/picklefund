...
export class MemberUsersController {
  ...
  async create(@CurrentUser() user: any, @Body() body: {
    ...
    return ok(await this.svc.create(user.clubId, user.userId, body), 'Tạo tài khoản thành công')
  }

  @Roles('CLUB_ADMIN', 'SUPER_ADMIN')
  @Post('bulk-create')
  async bulkCreate(@CurrentUser() user: any, @Body() body: {
    memberIds: string[]
    mustChangePassword?: boolean
    notificationEnabled?: boolean
  }) {
    return ok(await this.svc.bulkCreate(user.clubId, user.userId, body), 'Tạo tài khoản hàng loạt hoàn tất')
  }

  @Roles('CLUB_ADMIN', 'SUPER_ADMIN')
  @Post('preview-bulk')
  async previewBulk(@CurrentUser() user: any, @Body() body: { memberIds: string[] }) {
  ...
  @Roles('CLUB_ADMIN', 'SUPER_ADMIN')
  @Patch(':id/status')
  async updateStatus(@Param('id') id: string, @Body() body: { isActive: boolean }, @CurrentUser() user: any) {
    return ok(await this.svc.updateStatus(id, body.isActive, user.userId, user.clubId))
  }
}
