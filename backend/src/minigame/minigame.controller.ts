...
export class MinigameController {
  ...
  @Post()
  async create(@CurrentUser() user: any, @Body() body: { name: string; format: MinigameFormat; scheduledAt?: string; settings?: any }) {
    return ok(await this.svc.create(user.clubId, user.userId, {
      ...body,
  ...
  async addParticipants(@Param('id') id: string, @CurrentUser() user: any, @Body() body: { memberIds: string[] }) {
  ...
  async score(@Param('matchId') matchId: string, @CurrentUser() user: any, @Body() body: { scoreA: number; scoreB: number }) {
    return ok(await this.svc.updateMatchScore(matchId, user.clubId, body.scoreA, body.scoreB))
  }
