...
export class ExpensesService {
  ...
  async findOne(id: string, clubId: string) {
    const e = await this.prisma.livingExpense.findFirst({
      where: { id, clubId },
    });
  ...
  async create(clubId: string, userId: string, dto: CreateExpenseDto) {
    ...
    if (fundSource === 'COMMON') {
      ...
      if (!dto.allocationRule)
        throw new BadRequestException('allocationRule bắt buộc cho Quỹ Chung');
    }
    ...
    return this.prisma.livingExpense.update({
      ...
      data: {
        ...
        ...(dto.fundPeriodId !== undefined
          ...
          : {}),
        ...(dto.attendanceSessionId !== undefined
          ? { attendanceSessionId: dto.attendanceSessionId }
          : {}),
    ...
    return {
      common: Object.fromEntries(
        commonRules.map((rule, i) => [
          rule,
          {
            total: Number(commonResults[i]._sum.amount ?? 0),
            count: commonResults[i]._count,
          },
        ]),
      ),
      mini: {
