    ...
    <>
      ...
      <Section title="Sức khỏe hạ tầng" icon={<HeartPulse size={16} />}>
        <div className={`${GRID} grid-cols-2 sm:grid-cols-3 lg:grid-cols-5`}>
          ...
          <Kpi label="Hàng đợi việc" value={num(infra.queue?.pending)} sub={infra.queue ? `wf ${infra.queue.workflows} · ai ${infra.queue.aiActions}${infra.queue.notificationsFailed ? ` · noti lỗi ${infra.queue.notificationsFailed}` : ''}` : undefined} icon={<Server size={16} />} alert={infra.queue?.pending > 20 ? 'warning' : undefined} />
