...
export function MemberAttendance() {
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC]">
        <div className="sticky top-0 z-10 bg-white border-b border-slate-100 px-4 py-3">
          <div className="text-[17px] font-[800] text-slate-900">Lá»‹ch Tham Gia</div>
          {activePeriod && <div className="text-[12px] text-slate-400">{activePeriod.name} Â· {myMember?.fullName ?? 'ThÃ nh viÃªn'}</div>}
        </div>
        <div className="px-4 pt-4 pb-6 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              { label: 'Tá»· lá»‡', value: `${rate}%`, color: rate >= 80 ? 'text-emerald-600' : rate >= 60 ? 'text-amber-600' : 'text-red-500' },
              { label: 'Sáº¯p TG', value: `${periodSessions.filter(s => s.status === 'scheduled').length}`, color: 'text-amber-600' },
            ].map(k => (
          ...
          <div className="bg-white rounded-[14px] border border-slate-100 p-3 shadow-sm flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[13px] text-slate-600">Chi phÃ­ sÃ¢n cÃ¡ nhÃ¢n</span>
            </div>
          ...
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="TÃ¬m theo ngÃ y hoáº·c sÃ¢n..."
          ...
          {completedSessions.length > 0 && (
            <div className="bg-white rounded-[14px] border border-slate-100 p-3 shadow-sm">
              <div className="flex justify-between mb-1.5">
                <span className="text-[12px] font-[600] text-slate-600">Tá»· lá»‡ tham gia</span>
          ...
          {/* Session list */}
          {filtered.length === 0 ? (
            <div className="text-center py-12 text-slate-400 text-[14px]">ChÆ°a cÃ³ buá»•i táº­p nÃ o</div>
          ) : (
                ...
                return (
                  <div key={s.id} className={`bg-white rounded-[16px] border border-slate-100 p-4 shadow-sm ${!present && s.status === 'completed' ? 'opacity-60' : ''}`}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {s.status === 'scheduled'
                        ? <Badge variant="blue" dot>Sáº¯p TG</Badge>
                        : present
                          ? <Badge variant="green" dot>CÃ³ máº·t</Badge>
                          : <Badge variant="gray" dot>Váº¯ng</Badge>}
                    </div>
                    <div className="flex items-center gap-3 text-[12px] text-slate-500">
                      <span className="flex items-center gap-1"><MapPin size={11} />{s.courtName ?? 'SÃ¢n chÆ°a Ä‘áº·t'}</span>
                      {s.startTime && s.endTime && <span><Clock size={11} className="inline mr-0.5" />{s.startTime}â€“{s.endTime}</span>}
                    </div>
                    {present && costShare > 0 && (
                      <div className="mt-2 text-[12px] text-indigo-600 font-[600]">Chi phÃ­: {formatVND(costShare)}</div>
                    )}
                  </div>
                )
              })}
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-50">
      <PageHeader
        title="Lá»‹ch Tham Gia"
        subtitle={activePeriod ? `${activePeriod.name} Â· ${myMember?.fullName ?? 'ThÃ nh viÃªn'}` : 'ChÆ°a cÃ³ ká»³ quá»¹'}
      />
      ...
      <div className="p-6 max-w-[900px] mx-auto space-y-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white rounded-xl border border-slate-100 shadow-[var(--shadow-card)] p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Buá»•i tham gia</p>
            </div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Tá»· lá»‡: {rate}%</p>
          </div>
          <div className="bg-white rounded-xl border border-slate-100 shadow-[var(--shadow-card)] p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Sáº¯p diá»…n ra</p>
            </div>
            <p className="text-xl font-bold text-amber-600">{periodSessions.filter(s => s.status === 'scheduled').length} buá»•i</p>
            <p className="text-xs text-slate-500 mt-0.5">Trong ká»³ nÃ y</p>
          </div>
          <div className="bg-white rounded-xl border border-slate-100 shadow-[var(--shadow-card)] p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Chi phÃ­ sÃ¢n</p>
            </div>
            <p className="text-xl font-bold text-emerald-600">{formatVND(Math.round(courtCostPerSession))}</p>
            <p className="text-xs text-slate-500 mt-0.5">Pháº§n chia cÃ¡ nhÃ¢n</p>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="TÃ¬m theo ngÃ y hoáº·c sÃ¢n..."
        ...
        {completedSessions.length > 0 && (
          <div className="bg-white rounded-xl border border-slate-100 shadow-[var(--shadow-card)] p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-semibold text-slate-700">Tá»· lá»‡ tham gia</span>
        ...
        {filtered.length === 0 ? (
          <div className="bg-white rounded-xl border border-dashed border-slate-200 py-14 text-center">
            ...
            <p className="text-sm text-slate-400">ChÆ°a cÃ³ buá»•i táº­p nÃ o</p>
          </div>
        ) : (
            ...
            <table className="table-base">
              <thead>
                <tr>
                  ...
                  <th>NgÃ y</th>
                  <th>SÃ¢n</th>
                  <th className="text-center">Thá»i gian</th>
                  <th className="text-center">TÃ¬nh tráº¡ng</th>
                  <th className="text-right">Chi phÃ­ sÃ¢n</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((s, i) => {
                  ...
                  return (
                    <tr key={s.id} className={!present && s.status === 'completed' ? 'opacity-60' : ''}>
                      ...
                      <td className="text-slate-600 text-xs">{s.courtName ?? 'SÃ¢n chÆ°a Ä‘áº·t'}</td>
                      <td className="text-center text-xs text-slate-500">
                        {s.startTime && s.endTime ? `${s.startTime} â€“ ${s.endTime}` : 'â€”'}
                      </td>
                      <td className="text-center">
                        {s.status === 'scheduled'
                          ? <Badge variant="blue" dot>Sáº¯p diá»…n ra</Badge>
                          : present
                            ? <Badge variant="green" dot>CÃ³ máº·t</Badge>
                            : <Badge variant="gray" dot>Váº¯ng máº·t</Badge>}
                      </td>
