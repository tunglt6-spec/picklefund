      ...
      <div className="flex flex-col gap-3">
        {messages.map(m => (
          <div key={m.id} className={`flex gap-2 ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
            <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${m.role === 'lisa' ? '[background:var(--pf-primary-soft)]' : '[background:var(--pf-color-muted-soft)]'}`}>
    ...
    <div className="flex flex-col h-full [background:var(--pf-surface-muted)]">
      <div className="[background:var(--pf-surface)] border-b border-[color:var(--pf-border)] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold [color:var(--pf-text)]">Lisa AI</p>
