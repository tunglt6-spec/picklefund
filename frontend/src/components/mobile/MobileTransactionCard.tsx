...
export function MobileTransactionCard({ name, description, amount, type, fundSource, status }: MobileTransactionCardProps) {
  const isIncome = type === 'income'
  const formatted = new Intl.NumberFormat('vi-VN').format(Math.abs(amount)) + 'đ'

  return (
    <div className="bg-white rounded-[16px] border border-slate-100 px-4 py-3 flex items-center gap-3 shadow-sm">
      ...
      <div className="flex flex-col items-end flex-shrink-0">
        <span className={`text-[15px] font-[700] tabular-nums ${isIncome ? 'text-emerald-600' : 'text-red-500'}`}>
          {isIncome ? '+' : '−'}{formatted}
        </span>
        {status && (
          <span className="text-[11px] text-slate-400 mt-0.5">{status}</span>
        )}
      </div>
    </div>
  )
