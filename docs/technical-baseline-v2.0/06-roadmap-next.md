...
## 3d. Framework Governance

- **[APFG-01 — AI Platform Framework Governance Constitution](../framework/APFG-01-ai-platform-framework-governance-constitution.md)** ✅ **Accepted / Codex PASS / CLOSED** (tag `v2.1-apfg01-ai-platform-framework-governance`) — **Framework Governance ACTIVE**. Constitution **tối cao** product-agnostic của AI Platform Framework. Governance Precedence: APFG-01 → Release → Execution → Data → Security → AI → Product (tầng trên thắng khi xung đột). Chain: AI Platform Framework → APFG-01 → RELEASE-01 → EGOV-01 → EWR-01 → Execution Runtime → Products. **Docs-only**; mọi Program/Product kế thừa, không ghi đè. EGOV-01/EWR-01/ER/DataGOV/SGOV/AGOV là forward references (chưa triển khai). Không mở Execution / Epic 4.2.
- **[EGOV-01 — AI Platform Execution Governance Constitution](../execution/EGOV-01-ai-platform-execution-governance-constitution.md)** ✅ **Accepted / Codex PASS / CLOSED** (tag `v2.1-egov01-ai-platform-execution-governance`) — **Framework Execution Governance ACTIVE**. Execution Governance cấp Framework (kế thừa RELEASE-01/APFG-01; dưới RELEASE-01, trên EWR-01). Execution Precedence: Request → Policy → Approval → Readiness → Runtime → Result → Audit Trail (một chiều, không bypass). **Docs-only**; EWR-01/EEP-01/Execution Runtime/Policy·Approval·Audit·Rollback Engine/Agent Runtime là forward references (chưa hiệu lực). Không mở EWR-01/EEP-01/Epic 4.2.
- **[EWR-01 — Execution Readiness Framework](../execution/EWR-01-execution-readiness-framework.md)** ✅ **Accepted / Codex PASS / CLOSED** (tag `v2.1-ewr01-execution-readiness-framework`) — **Framework Execution Readiness ACTIVE**. Gate Model readiness cấp Framework (kế thừa EGOV-01; dưới EGOV-01, trên Execution Runtime). Readiness chain: Request → Policy → Approval → Security → Data → Dependency → Resource → Runtime → Execution Ready. 10 dimensions + decision model (READY/NOT_READY/EXPIRED/BLOCKED/DEFERRED/UNKNOWN); readiness expires + re-validate; deny by default. **Gate cuối trước implementation PickleFund v2.1**. **Docs-only**; EEP-01/Execution Runtime/các Engine là forward references. Không mở EEP-01/Epic 4.2.
...
Nguyên tắc phát triển, Delivery Pipeline, Workflow Gate, Execution Gate và Architecture Freeze được định nghĩa trong **GOV-01** (Rule 12, Rule 15, §6, §9). Tài liệu này không định nghĩa lại — xem [GOV-01](../governance/GOV-01-project-governance-baseline-v2.1.md). Quy trình vận hành increment/freeze bổ trợ: [Working Rules v2.1](../governance/WORKING-RULES-v2.1.md) ✅ **Accepted / Codex PASS / CLOSED** (tag `v2.1-working-rules`) — 7 rule (Increment Closure · Clean Working Tree · Freeze Dependency · Documentation Lifecycle · Transaction Boundary · Audit Scope Isolation · Lifecycle Synchronization Matrix) + Pre-Codex Audit / Pre-Commit / Transaction Boundary Checklist.
